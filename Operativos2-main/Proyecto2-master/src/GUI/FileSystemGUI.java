...
public class FileSystemGUI extends JFrame {
    ...
    private JButton btnCrearArchivo, btnEliminarArchivo, btnActualizarArchivo, btnRestaurarArchivo, 
                ...
                btnProcesarCola, btnLimpiarBuffer, btnVerGraficosBuffer, btnLeerArchivo; 
    ...
    public FileSystemGUI() {
        ...
        JMenuItem refMenuEstadisticas = menuEstadisticas;
        JMenuItem refMenuGraficos = menuGraficosBuffer; 
        ...
        // Agregar estas líneas para ajustar el ancho de las columnas
        infoTable.getColumnModel().getColumn(0).setPreferredWidth(100); // Nombre
        infoTable.getColumnModel().getColumn(1).setPreferredWidth(80);  // Bloque Inicial
        infoTable.getColumnModel().getColumn(2).setPreferredWidth(120); // Longitud
        infoTable.getColumnModel().getColumn(3).setPreferredWidth(80);  // Color
        ...
        infoPanel.setPreferredSize(new Dimension(380, 200));
        ...
        diskPanel.setPreferredSize(new Dimension(800, 300));        
        ...
        btnLeerArchivo.addActionListener(e -> leerArchivo());
        ...
        buttonPanel.add(btnLeerArchivo);
    ...
    private void mostrarGraficosBuffer() {
        if (!isAdmin) {
            JOptionPane.showMessageDialog(this, "🚫 Solo el Administrador puede ver gráficos del buffer.", 
    ...
    private void mostrarEstadisticas() {
            if (!isAdmin) {
            JOptionPane.showMessageDialog(this, "🚫 Solo el Administrador puede ver estadísticas.", 
                "Acceso Denegado", JOptionPane.WARNING_MESSAGE);
            return;
        }
        BufferCache buffer = fileSystem.getBuffer();
    ...
    private void leerArchivo() {
        ...
        String content = fileSystem.readFile(path, fileName, isAdmin ? "Administrador" : "Usuario");
    ...
    private void cambiarModoUsuario() { 
        ...
        btnCrearDirectorio.setEnabled(isAdmin);
        btnEliminarArchivo.setEnabled(isAdmin);
        btnActualizarArchivo.setEnabled(isAdmin);
        btnMoverArchivo.setEnabled(isAdmin);
        btnBorrarSistema.setEnabled(isAdmin);
        btnVerGraficosBuffer.setEnabled(isAdmin);
    ...
    private void actualizarInfoTabla(String nombre) {
        ...
        if (archivo != null) {
            if (archivo.blocks.getCabeza() != null) {
                ...
                // Iterate through ALL blocks and show each one as a separate row
                Nodo<Integer> actualBloque = archivo.blocks.getCabeza();
                int position = 0;

                while (actualBloque != null) {
                    infoTableModel.addRow(new Object[]{
                        archivo.name, 
                        actualBloque.dato,  // Show the actual block number
                        "#" + (position + 1) + " de " + archivo.blocks.contarElementos(), 
                        colorStr
                    });
