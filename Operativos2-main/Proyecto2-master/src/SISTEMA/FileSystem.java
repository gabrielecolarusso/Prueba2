...
public class FileSystem {
    ...
    private static final String CONTENTS_FILE = INFO_PATH + "file_contents.json"; 
    ...
    public String readFile(String path, String fileName, String usuario) {
        String fullPath = path.equals("/") ? "/" + fileName : path + "/" + fileName;
        String content = fileContents.getOrDefault(fullPath, "Contenido por defecto de: " + fileName);

        // Create a READ I/O request
        IORequest request = new IORequest(Operation.READ, path, fileName);
        request.setBlockPosition((int)(Math.random() * disk.getTotalBlocks()));
        IOProcess process = new IOProcess("READ_" + fileName, request, usuario);

        // Execute immediately for READ operations
    ...
    private void executeCreate(IORequest request, IOProcess process) {
        ...
        if (dir != null) {
            ...
            if (allocatedBlocks != null) {
                ...
                String fullPath = request.getPath().equals("/") ? "/" + request.getFileName() : request.getPath() + "/" + request.getFileName();
                fileContents.put(fullPath, "Contenido inicial de: " + request.getFileName() + "\nCreado el: " + java.time.LocalDateTime.now());
        ...
        if (dir != null) {
            ...
            while (actual != null) {
                if (actual.dato.name.equals(request.getFileName())) {
                    ...
                    // Update content in HashMap
                    String fullPath = request.getPath().equals("/") ? "/" + request.getFileName() : request.getPath() + "/" + request.getFileName();
                    fileContents.put(fullPath, request.getContent());
    ...
    public void guardarEnArchivo() {
        ...
        // Guardar también los contenidos de los archivos
        guardarContenidos();
    }
    ...
    public void cargarDesdeArchivo() {
        ...
        try (FileReader reader = new FileReader(FILE_NAME)) {
            ...
            // Cargar también los contenidos de los archivos
            cargarContenidos();
    ...
    private void cargarContenidos() {
        File file = new File(CONTENTS_FILE);
        if (!file.exists()) {
            return;
        }
