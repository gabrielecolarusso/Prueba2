...
public class ProcessImageCSV {
    ...
    public static void saveProcessesToCSV(List<ProcessImage> processes, String filePath) {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(filePath))) {
            writer.write("ID,Name,Type,Status,ProgramCounter,MemoryAddressRegister,Duration,Quantum,WaitingTime,InMemory,Instructions");
            writer.newLine();
    ...
    public static List<ProcessImage> readProcessesFromCSV(String filePath) {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(filePath))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                boolean inMemory = Boolean.parseBoolean(values[9]);

                List<Integer> instructions = parseInstructionsFromString(values[10]);
